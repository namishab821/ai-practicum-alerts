...
def fetch_web_search():
    listings = []
    API_KEY = os.environ.get("WEB_SEARCH_API_KEY")  # SerpAPI or Bing key
...
def send_email(html_body):
    ...
    # Friendly sender
    msg["From"] = "Practicum Alerts <{}>".format(YOUR_EMAIL)
    msg["To"] = TO_EMAIL
    ...
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
            ...
            result = server.sendmail(YOUR_EMAIL, TO_EMAIL, msg.as_string())
            if result == {}:
                print(f"Email successfully sent to {TO_EMAIL}")
            else:
                print(f"Email partially failed: {result}")
    except Exception as e:
